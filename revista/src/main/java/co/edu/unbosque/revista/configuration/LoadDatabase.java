...
public class LoadDatabase {
  ...
  @Value("${comentarista.password}")
  private String passwordComentador;
  ...
  CommandLineRunner initDatabase(UsuarioRepository userRepo, PasswordEncoder passwordEncoder) {	 
    return args -> {
      ...
      if (encontrado.isPresent()) {
        ...
        Usuario admin = new Usuario("admin", passwordEncoder.encode(passwordAdmin), Rol.ADMIN);
        userRepo.save(admin);
        log.info("Precargando usuario administrador");
      }
      Optional<Usuario> encontradoUsuario = userRepo.findByUsuario("normaluser");
      if (encontradoUsuario.isPresent()) {
        log.info("El usuario normal ya existe, omitiendo la creación del usuario normal...");
      } else {
        Usuario normalUser = new Usuario("normaluser", passwordEncoder.encode(passwordUsuario), Rol.USUARIO);
        userRepo.save(normalUser);
        log.info("Precargando usuario normal");
      }
      Optional<Usuario> encontradoEditor = userRepo.findByUsuario("editor");
      if (encontradoEditor.isPresent()) {
      ...
      if (encontradoComentarista.isPresent()) {
    	  log.info("El usuario normal ya existe, omitiendo la creación del comentador...");
      } else {
    	  Usuario editorUser = new Usuario("comentarista", passwordEncoder.encode(passwordComentador), Rol.COMENTADOR);
    	  userRepo.save(editorUser);
    	  log.info("Precargando editor");
      }
    };
